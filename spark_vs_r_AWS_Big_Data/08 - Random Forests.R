...
library(randomForest) #if not installed; install.packages('randomForest')
...
for(i in dataFiles){
  ...
  try({
    ...
    timing_wide = system.time({
      wide_model = randomForest::randomForest(Cancelled~.,data = tmp,ntree=1000,mtry=3,sampsize=1000000,nodesize=100)
    })
    accTab = wide_model$confusion
    rm(wide_model)
  ...
  try({
    timing_narrow = system.time({
      narrow_model = randomForest::randomForest(Cancelled~.,data = tmp,ntree=1000,mtry=3,sampsize=1000000,nodesize=100)
    })
    accTab = narrow_model$confusion
    rm(narrow_model)
    accuracy = (accTab[1,1] + accTab[2,2])/sum(accTab[1:2,1:2])
  ...
  #clean up
  try({
    readr::write_csv(dplyr::data_frame(File=i,Tool="R",NarrowTime=timing_narrow[3],WideTime=timing_wide[3],Technique="Random Forest"), "performance measures.txt", append=T)
    rm(tmp)
